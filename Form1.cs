...
namespace PetSimulator
{
    public partial class Form1 : Form
    {
        ...
        {
            // Form yüklendiğinde çalışacak kodlar (şu an boş)
        }
        ...
        {
            ownedAccessories.Add(new Accessory { Name = "Şapka", Icon = "^", Type = AccessoryType.Hat });
            ownedAccessories.Add(new Accessory { Name = "Papyon", Icon = "<>", Type = AccessoryType.Bowtie });
            ownedAccessories.Add(new Accessory { Name = "Gözlük", Icon = "O-O", Type = AccessoryType.Glasses });
        }
        ...
        private void WeatherTimer_Tick(object sender, EventArgs e)
        {
            if (random.Next(100) < 20)
            {
                ...
                string weatherMsg = currentWeather == WeatherType.Rainy ? "Yağmur yağıyor!" :
                                   currentWeather == WeatherType.Snowy ? "Kar yağıyor!" :
                                   currentWeather == WeatherType.Cloudy ? "Hava bulutlu" :
                                   "Hava güneşli!";

                trayIcon.ShowBalloonTip(2000, "Hava Değişti!", weatherMsg, ToolTipIcon.Info);
        ...
        private void StatsTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (age >= 140 && !achievements["week_old"])
            {
                ...
                ShowAchievementUnlocked("1 Haftalık", "Pet'in 1 hafta yaşadı!");
            }
            ...
            {
                trayIcon.ShowBalloonTip(2000, $"{petName} acıktı!", "Pet'iniz çok aç, yem verin!", ToolTipIcon.Warning);
                PlaySound("meow");
            }
            if (happiness < 20 && happiness % 6 == 0)
            {
                trayIcon.ShowBalloonTip(2000, $"{petName} sıkıldı!", "Pet'iniz mutsuz, onunla oynayın!", ToolTipIcon.Warning);
            }
        ...
        private void UpdatePetStage()
        {
            ...
            {
                string stageText = petStage == PetStage.Young ? "genç" : "yetişkin";
                trayIcon.ShowBalloonTip(3000, $"{petName} büyüdü!",
                    $"{petName} artık {stageText} bir kedi!", ToolTipIcon.Info);
                PlaySound("meow");
            }
        }
        ...
        private string GetAgeString()
        {
            ...
            string stage = petStage == PetStage.Baby ? "Bebek" :
                          petStage == PetStage.Young ? "Genç" : "Yetişkin";
        ...
        private void UpdateTrayIconStatus()
        {
            ...
            trayIcon.Text = $"{petName} - {status}\nAçlık:{hunger}% Mutluluk:{happiness}% Enerji:{energy}%";
        }
        ...
        {
            if (!soundEnabled) return; // Ses kapalıysa çık
        ...
        private void FeedPet(object sender, EventArgs e)
        {
            ...
            if (!achievements["first_feed"])
            {
                ...
                ShowAchievementUnlocked("İlk Besleme", "Pet'ini ilk kez besledin!");
            }
        ...
        private void PlayWithPet(object sender, EventArgs e)
        {
            ...
            if (!achievements["first_play"])
            {
                ...
                ShowAchievementUnlocked("İlk Oyun", "Pet'inle ilk kez oynadın!");
            }
            ...
            trayIcon.ShowBalloonTip(2000, "Yaşasın!",
        ...
        private void SleepPet(object sender, EventArgs e)
        {
            ...
            velocityY = 0;
            sleepCounter = 0;
            trayIcon.ShowBalloonTip(2000, "İyi uykular!",
                $"{petName} dinleniyor...",
                ToolTipIcon.Info);
            this.Invalidate();
        }
        ...
        private void ShowStatsMessage()
        {
            ...
            string weatherText = currentWeather == WeatherType.Sunny ? "Güneşli" :
                                currentWeather == WeatherType.Rainy ? "Yağmurlu" :
                                currentWeather == WeatherType.Snowy ? "Karlı" : "Bulutlu";

            MessageBox.Show(
                $">> {petName} - Pet İstatistikleri\n\n" +
                $"Açlık: {hunger}%\n" +
                $"Mutluluk: {happiness}%\n" +
                $"Enerji: {energy}%\n" +
                $"Yaş: {GetAgeString()}\n" +
                $"Yaşam Süresi: {timeLived.Days} gün {timeLived.Hours} saat\n" +
                ...
                $"Başarımlar: {completed}/6\n\n" +
                $"Durum: {condition}\n\n" +
                $">> Tuş Kontrolleri:\n" +
                $"I = Bakım Menüsü\n" +
                $"B = Top Fırlat\n" +
                $"S = Otur | P = Pati Ver | T = Takla At\n" +
                $"A = Aksesuar | H = Başarımlar",
                "Pet Simulator",
        ...
        private void UpdateTrayMenu()
        {
            ...
            trayMenu.Items[6].Text = $"Açlık: {hunger}%";
            trayMenu.Items[7].Text = $"Mutluluk: {happiness}%";
            trayMenu.Items[8].Text = $"Enerji: {energy}%";
            trayMenu.Items[9].Text = $"Yaş: {GetAgeString()}";

            // Ses menüsü güncelle
            if (soundMenuItem != null)
            {
                soundMenuItem.Text = soundEnabled ? "🔊 Sesler: Açık" : "🔇 Sesler: Kapalı";
            }
        }
        ...
        private void SavePetData()
        {
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(filePath))
                {
                    ...
                    writer.WriteLine(soundEnabled); // Ses ayarını kaydet
                }
            }
            catch { }
        }
        ...
        private void AskPetName()
        {
            ...
            {
                nameForm.Text = "Pet'ine İsim Ver!";
                ...
                okButton.Click += (s, ev) =>
                {
                    if (!string.IsNullOrWhiteSpace(textBox.Text))
                    {
                        ...
                        trayIcon.ShowBalloonTip(3000, $"Merhaba {petName}!",
                            "Pet'in artık hazır! Tuş kontrolleri:\nI=Menü B=Top S=Otur P=Pati T=Takla A=Aksesuar H=Başarımlar",
                            ToolTipIcon.Info);
                    }
                };
        ...
        private void ShowFeedMenu()
        {
            ...
            {
                menuForm.Text = $"{petName} - Bakım Menüsü";
                ...
                statsLabel.Text = $"Açlık: {hunger}%  |  Mutluluk: {happiness}%  |  Enerji: {energy}%";
                ...
                feedButton.Text = "Yem Ver (+35 Açlık)";
            ...
            using (Form accForm = new Form())
            {
                ...
                foreach (var acc in ownedAccessories)
                {
                    ...
                    btn.Click += (s, ev) =>
                    {
                        ...
                        trayIcon.ShowBalloonTip(1500, "Aksesuar Takıldı!",
                            $"{petName} artık {currentAccessory.Name} takıyor!", ToolTipIcon.Info);
                        accForm.Close();
                    };
        ...
        private void ShowAchievements()
        {
            ...
            achText += $" Top Ustası (10/10) - Yakalanan: {ballsCaught}\n";
            ...
            achText += " Numara Ustası\n";

            int completed = achievements.Values.Count(x => x);
            achText += $"\n>> Tamamlanan: {completed}/6";

            MessageBox.Show(achText, $"{petName} - Başarımlar", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void CheckAchievement(string achName)
        {
            if (achievements.ContainsKey(achName) && !achievements[achName])
            {
                if (achName == "ball_master" && ballsCaught >= 10)
                {
                    ...
                    ShowAchievementUnlocked("Top Ustası", "10 top yakaladın!");
                }
            }
        }
        ...
        {
            trayIcon.ShowBalloonTip(3000, $">> Başarım Kazanıldı: {name}", desc, ToolTipIcon.Info);
            PlaySound("meow");
        }
        ...
        private void BallTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (distance < 40)
            {
                ...
                PlaySound("purr");
                trayIcon.ShowBalloonTip(1500, "Yakaladı!", $"{petName} topu yakaladı! Top: {ballsCaught}", ToolTipIcon.Info);
        ...
        private void SpawnBall()
        {
            ...
            ballTimer.Start();
            PlaySound("meow");
            trayIcon.ShowBalloonTip(1000, "Top Zamanı!", $"{petName} topu kovalıyor!", ToolTipIcon.Info);
        }
        ...
        private void PerformTrick(PetTrick trick)
        {
            ...
            string trickName = trick == PetTrick.Sit ? "oturdu" : trick == PetTrick.Paw ? "pati verdi" : "takla attı";
            ...
            if (!achievements["trick_master"])
            {
                ...
                ShowAchievementUnlocked("Numara Ustası", "İlk numaranı yaptın!");
            }
        ...
        private void SetupTrayIcon()
        {
            ...
            trayMenu.Items.Add("-");
            trayMenu.Items.Add($"Açlık: {hunger}%", null, null);
            trayMenu.Items.Add($"Mutluluk: {happiness}%", null, null);
            trayMenu.Items.Add($"Enerji: {energy}%", null, null);
            trayMenu.Items.Add($"Yaş: {GetAgeString()}", null, null);
            trayMenu.Items.Add("-");

            // YENİ: Ses açma/kapama menüsü
            soundMenuItem = new ToolStripMenuItem(soundEnabled ? "🔊 Sesler: Açık" : "🔇 Sesler: Kapalı");
            soundMenuItem.Click += (s, ev) =>
            {
                soundEnabled = !soundEnabled;
                soundMenuItem.Text = soundEnabled ? "🔊 Sesler: Açık" : "🔇 Sesler: Kapalı";
                SavePetData();
                trayIcon.ShowBalloonTip(1000, "Ses Ayarı",
                    soundEnabled ? "Sesler açıldı!" : "Sesler kapatıldı!",
                    ToolTipIcon.Info);
            };
            trayMenu.Items.Add(soundMenuItem);
            trayMenu.Items.Add("-");

            trayMenu.Items.Add("İsim Değiştir", null, (s, ev) => { AskPetName(); });
            trayMenu.Items.Add("İstatistikler", null, (s, ev) => ShowStatsMessage());
            trayMenu.Items.Add("Başarımlar (H)", null, (s, ev) => ShowAchievements());
        ...
        private void ExitApp(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show(
                $"{petName}'den ayrılmak istediğinize emin misiniz?",
                "Pet Simulator",
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            if (e.CloseReason == CloseReason.UserClosing)
            {
                ...
                trayIcon.ShowBalloonTip(2000, $"{petName} hala burada!",
                    "Pet'iniz sistem tepsisinde çalışmaya devam ediyor.",
                    ToolTipIcon.Info);
            }
            else
            {
